...
object P67 {
  ...
  class Parser[+T](fn: String => Result[T]) extends (String => Result[T]) {
    ...
    def ~[U](that: => Parser[U]): Parser[(T,U)] =
      new Parser(input =>
        this.apply(input) match {
          case f @ Failure(_) => f
          case Success(result,rest) => that(rest) match {
            case f @ Failure(_) => f
    ...
    def |[U](that: => Parser[U]): Parser[Either[T,U]] =
      new Parser(input =>
        this.apply(input) match {
          case Failure(_) => that(input) match {
  ...
  lazy val letter: Parser[Char] = new Parser(input => {
    if (input.length == 0) Failure("no rest input")
      ...
      0) >= 'a' && input.charAt(0) <= 'z') Success(input.charAt(0),input.substring(1))
    else Failure(input.charAt(0) + " is not a letter")
  })
  def char(c:Char):Parser[Char] = new Parser(input => {
    if (input.length == 0) Failure("no rest input")
    else if (input.charAt(0) == c) Success(input.charAt(0), input.substring(1))
    else Failure(input.charAt(0) + " is not " + c)
  })
  lazy val tree:Parser[Tree[Char]] = (node | end) ^^ {case Left(v) => v case Right(v) => v}
  lazy val end:Parser[Tree[Char]] = new Parser(input => Success(End,input))
  lazy val node:Parser[Node[Char]] = letter ~ (char('(') ~ tree ~ char(',') ~ tree ~ char(')')).? ^^ {
