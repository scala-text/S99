...
object P96 {
  ...
  class Parser(fn : String => Result) extends (String => Result) {
    ...
    def ~(that: => Parser) : Parser =
      new Parser(input => this.apply(input) match {
        case f@Failure(_) => f
  ...
  lazy val letter : Parser = new Parser(input => {
      ...
      else if(input.charAt(0) >= 'A' && input.charAt(0) <= 'Z' || input.charAt(0) >= 'a' && input.charAt(0) <= 'z') Success(input.substring(1))
      else Failure(input.charAt(0) + " is not a letter")
    })
  lazy val digit : Parser = new Parser(input => {
      if(input.length == 0) Failure("no rest input")
      else if(input.charAt(0) >= '0' && input.charAt(0) <= '9') Success(input.substring(1))
