...
object P57 {
  implicit class RichTree[T](self: Tree[T]) {
    def addValue[U >: T](newValue: U)(implicit view: T => Ordered[U]): Node[U] = {
      self match {
        case Node(value, left, right) =>
          if(value < newValue) Node(value, left, right.addValue(newValue))
          else if(value > newValue) Node(value, left.addValue(newValue), right)
  ...
  object Tree {
    def fromList[T](elements: List[T])(implicit view: T => Ordered[T]): Tree[T] = {
